...
static void PHK_Stream_get_file(int dir,zval *ret_p,zval *uri_p,zval *mnt_p
	,zval *command_p,zval *params_p,zval *path_p,zval *cache_p
	TSRMLS_DC)
{
...
PHK_Cache_cache_id("node",4,Z_STRVAL_P(uri_p),Z_STRLEN_P(uri_p),&key
	TSRMLS_CC);
...
{
int dir;
zval *z_uri_p,* z_mnt_p,* z_command_p,* z_params_p, *z_path_p,*z_cache_p;

if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "bzz!z!z!z|z!",&dir
	,&z_uri_p,&z_mnt_p,&z_command_p,&z_params_p,&z_path_p,&z_cache_p)== FAILURE)
		EXCEPTION_ABORT_1("PHK_Stream::get_file: Cannot parse parameters",0);

PHK_Stream_get_file(0,return_value,z_uri_p,z_mnt_p,z_command_p,z_params_p
	,z_path_p,z_cache_p TSRMLS_CC);
}
...
static int PHK_Stream_close(php_stream *stream, int close_handle TSRMLS_DC)
{
PHK_STREAM_DATA *dp=stream->abstract;
...
{
zval z_key,z_cache,z_tmp,z_tmp_a,z_ssb,*args[8],z_uri,z_mode,z_size
	,z_mtime;
...
if (!dp->parse_done)
	...
	_phk_stream_parse_uri2(&z_uri,&(dp->z_command)
		,&(dp->z_params),&(dp->z_mnt),&(dp->z_path) TSRMLS_CC);
	if (EG(exception))
		{
	...
	if (Z_TYPE(dp->z_mnt) != IS_NULL)
		...
		if (EG(exception))
			{
			DBG_MSG1("do_stat: Invalid mount point (%s)",Z_STRVAL(dp->z_mnt));
			ABORT_PHK_STREAM_DO_STAT();
			}
		}
	}
...
if (Z_TYPE(z_ssb)!=IS_STRING) /* Cache miss */
	...
	ut_call_user_function_void(&CZVAL(PHK_Stream_Backend)
		,&CZVAL(get_stat_data),8,args TSRMLS_CC);
	if (EG(exception))
		{
...
if (found) memmove(ssb,Z_STRVAL(z_ssb),sizeof(*ssb));
	...
	{
	php_stream_wrapper_log_error(wrapper,dp->show_errors TSRMLS_CC
		,"%s: File not found",Z_STRVAL(dp->z_path));
	ABORT_PHK_STREAM_DO_STAT();
	}
...
static int PHK_Stream_seekdir(php_stream *stream, off_t offset, int whence
	, off_t *newoffset TSRMLS_DC)
{
PHK_STREAM_DATA *dp=stream->abstract;
HashTable *ht;
...
static php_stream *PHK_Stream_generic_open(int dir,php_stream_wrapper *wrapper
...
if (Z_TYPE(dp->z_mnt) != IS_NULL)
	...
	if (EG(exception))
			{
			DBG_MSG1("generic_open: Invalid mount point (%s)",Z_STRVAL(dp->z_mnt));
			ABORT_PHK_STREAM_OPEN();
			}
	}

PHK_Stream_get_file(dir,&(dp->z_data),&z_uri,&(dp->z_mnt),&(dp->z_command)
...
CLEANUP_PHK_STREAM_OPEN();	/*-- Cleanup */
...
static php_stream *PHK_Stream_openfile(php_stream_wrapper *wrapper
	...
	, php_stream_context *context STREAMS_DC TSRMLS_DC)
{
return PHK_Stream_generic_open(0,wrapper,uri,mode,options,opened_path,context
...
static php_stream *PHK_Stream_opendir(php_stream_wrapper *wrapper
	...
	, php_stream_context *context STREAMS_DC TSRMLS_DC)
{
return PHK_Stream_generic_open(1,wrapper,uri,mode,options,opened_path,context
	STREAMS_CC TSRMLS_CC);
}
...
{
/* Avoids mem leak */
php_stream_tidy_wrapper_error_log(&php_stream_phk_wrapper TSRMLS_CC);
