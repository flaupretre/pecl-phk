...
if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "zzzzz", &uri
	,&z_command,&z_params,&z_mnt,&z_path) == FAILURE) RETURN_NULL();

_phk_stream_parse_uri2(uri,z_command,z_params,z_mnt,z_path TSRMLS_CC);
}
	...
	switch(*p)
		...
		case '&':
			...
			if (z_params)
				...
				array_init(z_params);
				DBG_MSG1("parse_uri2: calling sapi_module.treat_data, ampp=<%s>"
					,ampp);
...
if (cmdp && (cmd_len==0)) cmd_len=p-cmdp; /* Command without argument */
...
if ((!cmdp) && (!mnt_len)) THROW_EXCEPTION_1("Empty URI",NULL);
...
if (z_command)
	{
	zval_dtor(z_command);
	if (cmdp) { ZVAL_STRINGL(z_command,cmdp,cmd_len,1); }
	else { ZVAL_NULL(z_command); }
	}

if (z_mnt)
	{
	zval_dtor(z_mnt);
	if (mnt_len) { ZVAL_STRINGL(z_mnt,urip,mnt_len,1); }
	else { ZVAL_NULL(z_mnt); }
	}

if (z_path)
	{
	zval_dtor(z_path);
	if (path_len) { ZVAL_STRINGL(z_path,path,path_len,1); }
	else { ZVAL_NULL(z_path); }
	}
}
