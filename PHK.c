...
static char *web_tunnel(PHK_Mnt_Info *mp, zval *path, int webinfo TSRMLS_DC);
...
if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "s", &mpp, &dummy_len
...
if (mp->mount_script_uri && (!(Z_LVAL_P(mp->flags) & PHK_F_NO_MOUNT_SCRIPT)))
	{
...
if (mp->plugin_class)
	{
	ut_new_instance(&(mp->plugin),mp->plugin_class,1,1,&(mp->mnt) TSRMLS_CC);
	if (EG(exception)) return;
	}
}
...
RETVAL_BOOL(ut_call_user_function_bool(&CZVAL(PHK_Proxy)
	,&CZVAL(file_is_package),1,&zp TSRMLS_CC));
}
...
RETVAL_BOOL(ut_call_user_function_bool(&CZVAL(PHK_Proxy)
	,&CZVAL(data_is_package),1,&zp TSRMLS_CC));
}

/*---------------------------------------------------------------*/

static int PHK_cache_enabled(PHK_Mnt_Info *mp, zval *command, zval *params
	, zval *path TSRMLS_DC)
{
if (mp->no_cache
	|| (Z_LVAL_P(mp->flags) & PHK_F_CREATOR)
	||(!PHK_Cache_cache_present(TSRMLS_C))) return 0;

if (Z_TYPE_P(mp->caching) != IS_NULL) return Z_BVAL_P(mp->caching);

if (PHK_Cache_caching_opcodes(TSRMLS_C) && ZVAL_IS_NULL(command)
	&& PHK_is_php_source_path(mp, path TSRMLS_CC)) return 0;
...
if (mp->umount_script_uri && (!(Z_LVAL_P(mp->flags) & PHK_F_NO_MOUNT_SCRIPT)))
	{
...
if ((Z_TYPE_P(zp)==IS_STRING) && zend_hash_find(Z_ARRVAL_P(mp->options)
	,Z_STRVAL_P(zp),Z_STRLEN_P(zp)+1,(void **)&zpp)==SUCCESS)
		{
		RETVAL_BY_REF(*zpp);
		}
}
...
if ((alen==1) && (Z_STRVAL_P(apath)[0]=='/')) return 1;

return ((plen >= alen) && ((alen==plen) || (Z_STRVAL_P(path)[alen]=='/'))
...
if (mp->web_access)
	...
	if (Z_TYPE_P(mp->web_access)==IS_ARRAY)
		...
		while (zend_hash_get_current_data_ex(ht
			,(void **)(&apath_pp),&pos)==SUCCESS)
			{
			if (Z_TYPE_PP(apath_pp)!=IS_STRING) continue;
			if (web_access_matches(*apath_pp,path TSRMLS_CC)) return 1;
...
if (!path) PHK_get_subpath(&tpath TSRMLS_CC);
else
	{
	tpath=*path;
...
if (last_slash && (Z_STRLEN(tpath)!=1)) ut_rtrim_zval(&tpath TSRMLS_CC);

if (!Z_STRLEN(tpath))
	{
	CLEANUP_WEB_TUNNEL();
	if (mp->web_run_script) return goto_main(mp TSRMLS_CC);
	else ut_http_301_redirect(&CZVAL(slash), 0 TSRMLS_CC);
	}
...
if ((!webinfo) && (!web_access_allowed(mp,&tpath TSRMLS_CC))
	...
	CLEANUP_WEB_TUNNEL();
	if (mp->web_run_script) return goto_main(mp TSRMLS_CC);
	else ut_http_403_fail(TSRMLS_C);
	}
...
if (S_ISDIR(ssb.sb.st_mode)) // Directory autoindex
	...
	if (last_slash)
		...
		for (pp=autoindex_fnames;*pp;p++)
			...
			if ((php_stream_stat_path(p,&ssb2)==0) && S_ISREG(ssb2.sb.st_mode))
				{
				found=1;
				zval_dtor(&tpath);
...
if ((!webinfo) && (PHK_is_php_source_path(mp,&tpath TSRMLS_CC)))
	...
	spprintf(&p,UT_PATH_MAX
		,"PHK_Mgr::instance('%s')->mime_header('%s');\nreadfile('%s');"
		,Z_STRVAL_P(mp->mnt),Z_STRVAL(tpath),Z_STRVAL(uri));
	}
...
if (!(type=PHK_mime_type(mp,path TSRMLS_CC))) return;
...
if (mp->mime_types && zend_hash_find(Z_ARRVAL_P(mp->mime_types)
	,Z_STRVAL(suffix),Z_STRLEN(suffix)+1,(void **)(&zpp))==SUCCESS)
	{
	zp=*zpp;
	}
else
	{
	if (zend_hash_find(Z_ARRVAL(mime_table),Z_STRVAL(suffix),Z_STRLEN(suffix)+1
		,(void **)(&zpp))==SUCCESS)
		{
		zval_dtor(&suffix);
		return *zpp;
		...
		{
		if (strstr(Z_STRVAL(suffix),"php"))
			zp=&CZVAL(application_x_httpd_php);
		else zp=NULL;
		}
	}
...
{
return (PHK_mime_type(mp,path TSRMLS_CC)==&CZVAL(application_x_httpd_php));
}
...
{
ut_call_user_function_void(PHK_Mgr_proxy_by_mp(mp TSRMLS_CC),&CZVAL(crc_check),
	0, NULL TSRMLS_CC);
}
...
if (mp->min_php_version
	&& php_version_compare(PHP_VERSION,Z_STRVAL_P(mp->min_php_version))<0)
	{
...
if (mp->max_php_version
	&& php_version_compare(PHP_VERSION,Z_STRVAL_P(mp->max_php_version))>0)
	{
...
if (mp->plugin) { RETVAL_BY_REF(mp->plugin); }
}
...
if ((Z_TYPE_P(zp)==IS_STRING) && zend_hash_find(Z_ARRVAL_P(mp->build_info)
	,Z_STRVAL_P(zp),Z_STRLEN_P(zp)+1,(void **)&zpp)==SUCCESS)
	{
	RETVAL_BY_REF(*zpp);
	}
...
if (!mp->backend)
	{
	ut_new_instance(&(mp->backend),&CZVAL(PHK_Backend),1,1,&phk TSRMLS_CC);
	}
...
if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "zz", &method,&call_args
	) == FAILURE) EXCEPTION_ABORT_1("Cannot parse parameters",0);
...
PHK_Mnt_Info *mp;
zval *file,*cmd,*ret,**argv1_pp;
char *p;
struct stat dummy_sb;

DBG_MSG("Entering Prolog");

if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "zzz", &file, &cmd, &ret
	) == FAILURE) EXCEPTION_ABORT_1("Cannot parse parameters",0);
...
if (cli)
	{
	(void)zend_alter_ini_entry("display_errors",sizeof("display_errors"),"1"
		,1,PHP_INI_USER, PHP_INI_STAGE_RUNTIME);
	if (!PG(safe_mode)) (void)zend_alter_ini_entry("memory_limit"
		,sizeof("memory_limit"),"1024M",5,PHP_INI_USER, PHP_INI_STAGE_RUNTIME);
	}
...
if (cli)
	...
	if (Z_LVAL_P(SERVER_ELEMENT(argc))>1)
		{
		zend_hash_index_find(Z_ARRVAL_P(SERVER_ELEMENT(argv)),(ulong)1
			,(void **)&argv1_pp);
		if (Z_STRVAL_PP(argv1_pp)[0]=='@')
			{
			ZVAL_LONG(ret,ut_call_user_function_long(mp->instance
				,&CZVAL(builtin_prolog),1,&file TSRMLS_CC));
			return;
			}
		}
...
static zend_function_entry PHK_functions[]= {
	...
	PHP_ME(PHK,file_is_package,UT_1arg_arginfo,ZEND_ACC_PUBLIC|ZEND_ACC_STATIC)
	PHP_ME(PHK,data_is_package,UT_1arg_arginfo,ZEND_ACC_PUBLIC|ZEND_ACC_STATIC)
	...
	PHP_ME(PHK,accelerator_is_present,UT_noarg_arginfo,ZEND_ACC_PUBLIC|ZEND_ACC_STATIC)
	PHP_ME(PHK,build_info,UT_noarg_ref_arginfo,ZEND_ACC_PUBLIC)
	PHP_ME(PHK,subpath_url,UT_1arg_arginfo,ZEND_ACC_PUBLIC|ZEND_ACC_STATIC)
	PHP_ME(PHK,get_subpath,UT_noarg_arginfo,ZEND_ACC_PUBLIC|ZEND_ACC_STATIC)
	PHP_ME(PHK,__call,UT_2args_arginfo,ZEND_ACC_PUBLIC)
	PHP_ME(PHK,prolog,PHK_prolog_arginfo,ZEND_ACC_PUBLIC|ZEND_ACC_STATIC)
    {NULL, NULL, NULL, 0, 0 }
	};
...
PHK_NEW_ZP();
ZVAL_LONG(zp,PHK_F_KEEP_OPEN);
PHK_ADD_ZP_CONST("F_KEEP_OPEN");
...
{
zend_class_entry ce,*PHK_sc_entry;

/*--- Init class and set constants*/

INIT_CLASS_ENTRY(ce, "PHK", PHK_functions);
PHK_sc_entry=zend_register_internal_class(&ce TSRMLS_CC);
set_constants(PHK_sc_entry);
